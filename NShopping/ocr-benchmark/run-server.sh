#!/usr/bin/env bash
# Build the image and run the PERSISTENT-SERVER benchmark (true per-photo latency,
# model kept resident). Run this ON THE LINUX BOX WITH THE RX 580.
#
#   ./run-server.sh /path/to/PXL_20260802_194940316.jpg
#
# Tune via env vars, e.g.:  RES=1024 ITERS=8 ./run-server.sh photo.jpg
set -euo pipefail

IMG_FILE="${1:?Usage: ./run-server.sh <path-to-photo.jpg> [hf-repo:quant]}"
HF_REPO="${2:-}"
