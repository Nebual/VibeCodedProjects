...
# Usage (from anywhere): ssh gman@gman '/servers/nmediatrack/NMediaTrack/bin/gman-deploy.sh'
# Or via the repo: pnpm gman-deploy
set -e
cd /servers/nmediatrack/NMediaTrack
...
screen -S nmediatrack -p 0 -X stuff $'\003'
sleep 0.5
# ...then start the new one and press Enter
screen -S nmediatrack -p 0 -X stuff 'pnpm start\n'
